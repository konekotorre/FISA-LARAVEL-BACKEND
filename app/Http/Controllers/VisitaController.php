...
class VisitaController extends Controller
{
    public function calcularTareas($visitas)
    {
        for ($i = 0; $i < count($visitas); $i++) {
            ...
            $tareasHechas = DB::table('tareas')
            ->select('tareas.id as tareasHechas')
            ->where('tareas.visita_id', $visitas[$i]->id)
                ->whereIn('tareas.estado_id', [3, 5])
                ->count();

            $visitas[$i]->tareasHechas = $tareasHechas;
    ...
    public function search(Request $request)
    {
        ...
        $fecha_inicio = $request->fecha_inicio ? Carbon::parse($request->fecha_inicio)->setTimezone('America/Bogota')->format('Y/m/d') : null;
        $fecha_fin = $request->fecha_fin ? Carbon::parse($request->fecha_fin)->setTimezone('America/Bogota')->format('Y/m/d') : null;
